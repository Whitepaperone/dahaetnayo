...
class change_Activity : AppCompatActivity(){
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        var count: Int = 0
        var step: Int = 0
        tv_message.setText("현재 비밀번호를 입력해주세요")
        ...
        fun check(){
            if(password.length == 4&&password.equals(double_check)){
                ...
                //설정창으로 이동하는 거 추후 추가 예정
                val myIntent = Intent(this, password_Activity::class.java)
                startActivity(myIntent)

            }
            else if(password.length == 4&&step == 0){
                if(password.equals(settings.getString("passwd",""))){
                    Handler().postDelayed({
                        count = 0
                        step++
                        password=""
                        tv_message.setText("변경할 비밀번호를 입력해주세요")
                        box(count, "")
                    }, 200L)
                }
                else{
